...
static void store_open(eos_settings_store_t *st)
{
    ...
    memset(&F, 0, sizeof F);
    F.apply_ret_key = -1;
    snprintf(F.cur_ssid, sizeof F.cur_ssid, "ExampleNet");
...
static void t_wifi_routes(void)
{
    ...
    CKS(val, "ExampleNet", "  from eos_net, not from the store");
    F.psk_stored = true;
...
static void t_settings_endpoint(void)
{
    ...
    HAS(r.body, "\"wifi.ssid\":\"ExampleNet\"", "  the ssid, read from eos_net");
    ...
    HAS(r.body, "\"wifi.ssid\":\"ExampleNet\"",
