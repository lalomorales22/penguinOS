...
static void test_wifi_forget_and_status(void)
{
    ...
    memcpy(FK.net.ssid, "ExampleNet", 10);
    FK.net.ssid_len = 10;
...
static void test_web_contract(void)
{
    ...
    CK(has_kv(r.body, "ssid", "\"ExampleNet\""),     "net/status: ssid at the top level");
...
static void test_the_walkthrough(void)
{
    ...
    ap_set(0, "ExampleNet", 10, -47, 6, EOS_HTTPD_AUTH_WPA2);
    FK.wifi_n = 1;
    CKI(callj("GET", "/api/wifi/scan", NULL, &r), 200, "3. the cached scan is there immediately");
    CK(strstr(r.body, "ExampleNet") != NULL, "3. the network is in the list");

    // The user types the wrong password.
    CKI(callj("POST", "/api/wifi/connect",
              "{\"ssid\":\"ExampleNet\",\"psk\":\"wrongpass\"}", &r), 202,
    ...
    CKI(callj("POST", "/api/wifi/connect",
              "{\"ssid\":\"ExampleNet\",\"psk\":\"rightpass\"}", &r), 202,
