...
| `kernel/hal` | 4 headers + ST7789 backend + storage backend + tests | yes | **109 + 276 checks** | display yes, storage no | The event ring now carries pointer events beside keys. `EOS_FS_FAT` is still not implemented. The ESP32-S3-Touch-LCD-1.47 now HAS verified card pins - a 15GB card mounted at 20MHz on SPI3, so it does not even contend with the panel - which removes the reason this was deferred. |
| `boards/` | schema + 8 profiles | yes | validator + generator | n/a | All 8 validate, generate headers that compile and link, and regeneration is byte-reproducible. FOUR are now verified on hardware - cyd-2432s024n, lafvin-c6-lcd-147, waveshare-c6-lcd-13, waveshare-s3-touch-lcd-147 - across THREE silicon families. |
...
| **`render.heap_budget_bytes` is measured on the four verified boards and still a guess on the other four.** Measure it with penguinOS ITSELF: a bare probe on the CYD reports 281,612 free while the OS at `app_main` reports 148,188, and sizing the display from the probe figure asked for 281,280 contiguous bytes out of a 110,592-byte largest block and booted the board blind. The free TOTAL is not the constraint; the largest BLOCK is. The wavvy figure (64 KB) contradicts the owner's own measurement — `../tft-videos/CLAUDE.md` records free heap at 48–55 KB on that exact firmware. `eos_display_init()` sizes off this number, so a wrong value is an OOM at boot, not a degradation. | Needs a real heap dump on real silicon. Cannot be derived. | First bring-up. |
